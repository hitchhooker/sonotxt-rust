...
pub async fn validate_session(db: &PgPool, token: &str) -> Result<User> {
    ...
    let row: Option<(Uuid, Option<String>, Option<String>, Option<String>, Option<String>, f64, Option<String>)> = sqlx::query_as(
        r#"
        SELECT u.id, u.nickname, u.email, u.public_key, u.wallet_address, u.balance::float8, u.avatar
        FROM sessions s
...
/// Update user avatar
pub async fn update_avatar(db: &PgPool, user_id: Uuid, avatar: Option<&str>) -> Result<()> {
    // Validate avatar name if provided
    const ALLOWED_AVATARS: &[&str] = &[
        "haru", "hiyori", "mao", "mark", "natori", "rice", "wanko",
    ];
    if let Some(a) = avatar {
        if !ALLOWED_AVATARS.contains(&a) {
            return Err(ApiError::InvalidRequest("invalid avatar".to_string()));
        }
    }
