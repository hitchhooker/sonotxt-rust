...
async fn handle_checkout_complete(state: &AppState, session: &serde_json::Value) -> Result<()> {
    ...
    // Convert fiat to TXT at the fiat rate (with premium)
    let price = match &state.sono {
        Some(sono) => sono.price.read().await.clone(),
        None => crate::services::sono::PriceInfo::default(),
    };
    let txt_amount = crate::services::billing::fiat_to_txt(amount, &price);

    // Credit TXT to user
    crate::services::billing::credit_txt(&state.db, account_id, txt_amount, "stripe", &payment_id).await?;

    // Also record in deposits table
    credit_deposit(&state.db, deposit_id, account_id, amount, "stripe", &payment_id).await?;

    tracing::info!(
        "stripe {} credited {} TXT ({} {}) to {}",
        payment_id, crate::services::billing::format_txt(txt_amount), amount, currency, account_id
    );
