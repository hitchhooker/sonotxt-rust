...
pub const FREE_TIER_DAILY_LIMIT: i32 = 300;
pub const FREE_TIER_LOGGED_IN_LIMIT: i32 = 1000;
...
impl FromRequestParts<std::sync::Arc<AppState>> for TtsUser {
    ...
    async fn from_request_parts(
            ...
            parts.extract::<TypedHeader<Authorization<Bearer>>>().await
        {
            ...
            // check user sessions table (wallet/magic-link/pubkey auth)
            let token_hash = {
                use sha2::{Sha256, Digest};
                let mut h = Sha256::new();
                h.update(b"sonotxt-session-v1");
                h.update(token.as_bytes());
                hex::encode(h.finalize())
            };

            let session_row: Option<(uuid::Uuid, Option<String>)> = sqlx::query_as(
                r#"
            ...
            if let Some((user_id, wallet_address)) = session_row {
                // cache session as api key format for future lookups
                let api_key = super::api_key::ApiKey {
                    key: token.to_string(),
                    account_id: user_id,
                    created_at: chrono::Utc::now(),
...
pub async fn check_free_tier_limit(
    ...
    check_free_tier_limit_with(db, ip_hash, chars_needed, FREE_TIER_DAILY_LIMIT).await
}

pub async fn check_free_tier_limit_with(
    db: &sqlx::PgPool,
    ip_hash: &str,
    chars_needed: i32,
    limit: i32,
    ...
    let remaining = limit - row.chars_used;
